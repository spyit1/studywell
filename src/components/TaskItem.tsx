...
const TaskItem = ({ task }: { task: Task }) => {
  ...
  const doneStyle = task.isDone 
    ? "opacity-60 line-through bg-green-50 dark:bg-green-900/30 shadow-inner" 
    : "bg-white dark:bg-gray-900 hover:shadow-lg dark:hover:shadow-gray-700/50";
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start">
        <h2 className={`text-xl font-semibold ${task.isDone ? 'text-gray-500 dark:text-gray-500' : 'text-gray-800 dark:text-gray-50'}`}>
          {task.title}
        </h2>
